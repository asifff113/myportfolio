 ...
 * Simplified on mobile: fewer blobs, no spinning halos, no neon dots
 */
...
export default function AnimatedBackground({ variant = "default" }: AnimatedBackgroundProps) {
  ...
  return (
    <div className="fixed inset-0 overflow-hidden -z-10 pointer-events-none">
      ...
      {/* Aurora blobs — show only 3 on mobile (static), all 6 on desktop */}
      <div className="aurora-blob top-[-10rem] left-[-6rem] h-[36rem] w-[36rem] bg-violet-600/30" />
      <div className="aurora-blob delay-1 top-[5%] right-[-8rem] h-[32rem] w-[32rem] bg-orange-500/20 hidden md:block" />
      <div className="aurora-blob delay-2 bottom-[-8rem] left-[10%] h-[34rem] w-[34rem] bg-cyan-500/25" />
      <div className="aurora-blob delay-1 bottom-[-10rem] right-[8%] h-[30rem] w-[30rem] bg-emerald-500/20 hidden md:block" />
      <div className="aurora-blob top-[35%] left-[45%] h-[28rem] w-[28rem] bg-rose-500/15" />
      <div className="aurora-blob delay-2 top-[60%] left-[20%] h-[24rem] w-[24rem] bg-indigo-500/20 hidden md:block" />

      {/* Rotating conic halo ring — desktop only */}
      <div className="absolute inset-0 items-center justify-center opacity-25 hidden md:flex">
      ...
      {/* Drifting neon dots — desktop only */}
      ...
      {/* Decorative light beams — desktop only */}
